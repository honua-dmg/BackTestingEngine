...
class Algo1(CleanData):
    ...
    def transform(self,message): # NOTE: keeping track of last_traded_vol here might be dangerous in the event of a crash, keep this in mind.
        ...
        buy_sq = float(np.dot(buy_prices-ltp, (buy_prices-ltp).T))
        sell_sq = float(np.dot(sell_prices-ltp, (sell_prices-ltp).T))
        if buy_sq == sell_sq:
            # depth unavailable/symmetric (e.g. spot index with no real order book)
            # fall back to price direction: down tick = selling pressure ('b'), up tick = buying pressure ('s')
            ltp_type = 'b' if (self.last_ltp is not None and ltp < self.last_ltp) else 's'
        else:
            ltp_type = 'b' if buy_sq < sell_sq else 's'
        self.last_ltp = ltp
